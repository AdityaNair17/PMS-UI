...
const router = jsonServer.router('./json/sample.json')
const userdb = JSON.parse(fs.readFileSync('./json/userdb.json', 'UTF-8'))
...
function userDetails({email, password}){
  ...
  const user = {
    "firstName" : userdb.users[index].firstName,
    "lastName" : userdb.users[index].lastName,
...
fs.readFile("./json/userdb.json", (err, data) => {  
    ...
    var writeData = fs.writeFile("./json/userdb.json", JSON.stringify(data), (err, result) => {  // WRITE
        if (err) {
          const status = 401
          const message = err
          res.status(status).json({status, message})
...
server.post('/auth/login', (req, res) => {
  ...
  const status = 200;
  const user = userDetails({email,password});
